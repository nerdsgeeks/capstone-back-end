  ...
  //  const hashedPassword = await bcrypt.hash( password, 10);
   ...
   const match = await bcrypt.compare(password, result.recordset[0].Password);
    ...
    console.log(userID);
      ...
      if (match) {
const accessToken  = jwt.sign({"userid": userID}, process.env.ACCESS_TOKEN_SECRET,{ expiresIn: "1h"} );
const refreshToken = jwt.sign({"userid": userID}, process.env.REFRESH_TOKEN_SECRET,{ expiresIn: "24h"} );

 let updaterefreshToken = await pool.request()
 .input('ID',sql.Int, userID)
      .input('refreshToken',sql.VarChar,refreshToken)
     .execute('UpdateToken');

          res.cookie('jwt', refreshToken, { httpOnly: true, sameSite: 'None', secure: true, maxAge: 24 * 60 * 60 * 1000 });
          res.json({ accessToken, FirstName,LastName,EmployeeType});
      } else {
